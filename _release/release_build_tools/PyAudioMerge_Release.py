...
#     # Get the absolute path to the resource, for PyInstaller and normal development
#     try:
#         # PyInstaller creates a temp folder and stores path in _MEIPASS
#         base_path = sys._MEIPASS
#     except AttributeError:
#         # Use the default temporary directory provided by the OS
...
def resource_path(relative_path):
    try:
        ...
        # Use the script's directory for development
...
class AudioMerge:
    def __init__(self, root):
        self.root = root
        self.root.title("AudioMergeMKV_Release - Version 1.0")
        ...
        self.audio_tracks_file = resource_path("presets/default_preset.json")

        # Set the base path to the executable's directory or the script directory
        ...
        # Create export and preset folders in the base directory
        self.export_folder = os.path.join(base_dir, 'output')
        self.preset_folder = os.path.join(base_dir, 'presets')
        self.default_preset_path = resource_path("presets/default_preset.json")
        ...
        # Copy default_preset.json if it doesn’t exist in the presets folder
        target_preset_path = os.path.join(self.preset_folder, 'default_preset.json')
        if not os.path.exists(target_preset_path):
            shutil.copyfile(self.default_preset_path, target_preset_path)
    ...
    def create_widgets(self):
        ...
        self.start_time_entry.insert(0, "00:00:00")  # Set default value
    ...
    @staticmethod
    def read_audio_tracks(relative_path):
    ...
    def execute_ffmpeg(self, input_filepath, tracks, volumes, indices, first_2_letters):
        ...
        if getattr(sys, 'frozen', False):
            ...
            ffmpeg_dir = os.path.join(sys._MEIPASS, "ffmpeg")  # Assuming "ffmpeg" folder is in the executable
        ...
        try:
            subprocess.run(ffmpeg_command, check=True, startupinfo=startupinfo)
